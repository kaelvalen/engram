...
Reads output/sgms/sgms-spike-{sgms,B1,B2,B3}/seed*.json and reports, per model
and context: mean ± std recall over seeds, final min expert utilization, and
the gate verdict:

    PASS  if sgms-spike ≥ B1 AND ≥ max(B2, B3) on recall @ 4k context
          AND final min expert utilization ≥ 10% (no hard collapse).

Usage:
    python scripts/sgms_spike_gate.py [--runs-dir output/sgms] [--gate-context 4096]
"""
...
def _collect(runs_dir: Path) -> dict:
    data = {}
    for kind in KINDS:
        seeds = sorted((runs_dir / f"sgms-spike-{kind}").glob("seed*.json"))
        if not seeds:
            # also accept the direct experiment dir (sgms-spike for kind=sgms)
            alt = (runs_dir / ("sgms-spike" if kind == "sgms" else f"sgms-spike-{kind}")).glob(
                "seed*.json"
            )
            seeds = sorted(alt)
        if seeds:
...
def main() -> int:
    ...
    ap.add_argument("--runs-dir", default="output/sgms")
    ...
    # ---- gate verdict ----
    if "sgms" not in summary or not any(k in summary for k in ("B1", "B2", "B3")):
        print("\nGATE: INCONCLUSIVE (missing sgms or baselines)")
        return 2
    gate_ctx = args.gate_context
    sgms_acc = torch.tensor(summary["sgms"]["by_ctx"].get(gate_ctx, [0.0])).mean().item()
    rivals = {}
    for k in ("B1", "B2", "B3"):
        if k in summary:
            rivals[k] = torch.tensor(summary[k]["by_ctx"].get(gate_ctx, [0.0])).mean().item()
    min_util = min(summary["sgms"]["min_util"]) if summary["sgms"]["min_util"] else 0.0

    ok_quality = all(sgms_acc >= v for v in rivals.values()) and rivals
    ok_collapse = min_util >= 0.10
    print(
        f"\nrecall@{gate_ctx}: sgms={sgms_acc:.3f} | "
