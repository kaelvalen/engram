...
def load_model(checkpoint_path: str, device: torch.device) -> PRISMForClassification:
    ...
    print(f"Loaded checkpoint — epoch: {epoch}, val_acc: {val_acc:.4f}" if isinstance(val_acc, float)
          else f"Loaded checkpoint — epoch: {epoch}")
    return model

@torch.no_grad()
def infer_single(model: PRISMForClassification, image_path: str, device: torch.device, patch_size: int = 4):
...
@torch.no_grad()
def eval_cifar(model: PRISMForClassification, data_root: str, device: torch.device, batch_size: int = 128):
    ...
    loader = DataLoader(ds, batch_size=batch_size, shuffle=False,
                        num_workers=4, collate_fn=PatchCollator(patch_size=4))
    ...
    for c in range(10):
        ...
        print(f"  {CIFAR10_CLASSES[c]:12s}  {acc:6.2f}%  ({per_class_correct[c]}/{per_class_total[c]})")
...
def main():
    ...
    parser.add_argument("--cifar-test",  action="store_true",      help="CIFAR-10 test seti değerlendirme")
