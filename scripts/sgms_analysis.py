"""Generate the SGMS §7 analysis report from a finished spike run.

Loads a checkpoint written by sgms.train (seed{N}.pt) plus the run's
...
Usage:
    python scripts/sgms_analysis.py output/sgms/sgms-spike-sgms/seed0.pt \
        output/sgms/sgms-spike-sgms/seed0.json output/sgms/analysis/seed0
"""
...
import torch
from sgms.analysis import generate_report
...
def main() -> int:
    ...
    ap.add_argument("checkpoint", help="seed{N}.pt written by sgms.train")
    ...
    model = build_model(config["model"].get("kind", "sgms"), cfg, config["task"]["vocab_size"])
