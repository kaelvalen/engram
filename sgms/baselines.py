...
def _build_engram_block(name: str, cfg: SGMSConfig) -> nn.Module:
    if name == "ssd":
        return SSDBlock(
...
def build_model(kind: str, cfg: SGMSConfig, vocab_size: int) -> nn.Module:
    """Single entry point for SGMS and every registered baseline."""
    if kind == "sgms":
        return SGMSLM(cfg, vocab_size)
    if kind in ("B1", "B2", "B3"):
        return HybridLM(kind, cfg, vocab_size)
    if kind == "B4":
        return SGMSLM(replace(cfg, router_mode="uniform"), vocab_size)
    if kind == "B5":
        return SGMSLM(replace(cfg, router_mode="random"), vocab_size)
    raise ValueError(f"unknown model kind {kind!r} (sgms | {BASELINE_KINDS})")
