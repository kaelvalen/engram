"""SGMS language model: token embedding + SGMS blocks + LM head."""
...
class SGMSLM(nn.Module):
    """Causal LM over a stack of SGMS blocks (spec §4 configs)."""

    def __init__(self, cfg: SGMSConfig, vocab_size: int):
        super().__init__()
        self.cfg = cfg
        self.vocab_size = vocab_size
        self.embed = nn.Embedding(vocab_size, cfg.hidden_dim)
        self.blocks = nn.ModuleList([SGMSBlock(cfg, i) for i in range(cfg.num_layers)])
