...
Usage:
    python -m sgms.train --config configs/sgms/spike.yaml [--seed 0]
"""
...
def train_one(config: dict, seed: int, device: str = "cpu") -> dict:
    ...
    is_sgms = hasattr(model, "blocks") and kind in ("sgms", "B4", "B5")
    history = []
    prev_indices = None
    t0 = time.time()
    for step in range(steps):
        ...
        with torch.autocast(device_type=device_type, dtype=torch.bfloat16, enabled=use_amp):
            ...
            if is_sgms:
                aux, parts = sgms_auxiliary_loss(out["routings"], cfg.lambda_bal, cfg.lambda_z)
                loss = task_loss + aux
        ...
        opt.step()
        if is_sgms and cfg.use_surprise_predictor:
    ...
    ckpt_dir = Path(config.get("log_dir", "output/sgms")) / config.get("experiment", "run")
...
def main(argv=None):
    ap = argparse.ArgumentParser(description="SGMS spike trainer (MQAR)")
    ...
    log_dir = Path(config.get("log_dir", "output/sgms")) / config.get("experiment", "run")
