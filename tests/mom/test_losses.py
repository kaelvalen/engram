...
def test_auxiliary_loss_lambda_scaling():
    ...
    total, parts = mom_auxiliary_loss([out], lambda_bal=1e-2, lambda_z=1e-3)
    # L_bal = 1, L_z = log(2)²
...
def test_auxiliary_loss_zero_lambdas():
    out = _uniform_routing()
    total, _ = mom_auxiliary_loss([out], lambda_bal=0.0, lambda_z=0.0)
...
def test_auxiliary_loss_gradient_reaches_router_weight():
    ...
    total, _ = mom_auxiliary_loss([out], lambda_bal=1e-2, lambda_z=1e-3)
