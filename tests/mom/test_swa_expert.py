"""SWA expert for MoM (spec §3.4 SWA bullet, v2 activation).
...
def test_v1_swa_config_now_activates():
    ...
    model = build_model("mom", cfg, vocab_size=32)
...
def test_mom_block_with_swa_masked_equivalence(experts):
    cfg = _cfg(experts=experts)
    torch.manual_seed(0)
    block = MoMBlock(cfg, 0).to(torch.float64).eval()
...
def test_mom_block_with_swa_state_passing():
    cfg = _cfg(experts=("gdr", "swa"), num_layers=2)
    torch.manual_seed(0)
    from mom.model import MoMLM

    model = MoMLM(cfg, vocab_size=32).to(torch.float64).eval()
