"""Core spec §9 gate: dense-masked MoMBlock ≡ sequential token-by-token
reference, float64, rtol=1e-10.

The reference (mom.reference.sequential_block_reference) routes every token
...
def _block(dtype=torch.float64, seed=0, **overrides):
    ...
    return MoMBlock(cfg, layer_idx=0).to(dtype).eval()
