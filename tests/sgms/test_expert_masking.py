"""Additive write-mask flags on the ENGRAM mixers (SGMS spec §3.4).

The SGMS router selects a memory primitive per token.  Under dense-masked
...
GDR  — ``k, v, β`` are masked, so the transition ``(I − β̃ k̃ k̃ᵀ)`` collapses
       ...
       miss when ``freeze_on_mask=True`` (SGMS's default for GDR).
