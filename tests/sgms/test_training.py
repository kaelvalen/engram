...
def test_unknown_baseline_rejected():
    ...
    with pytest.raises(ValueError):
        build_model("B9", SGMSConfig(), vocab_size=32)
...
def test_b4_b5_are_sgms_with_frozen_routers():
    b4 = build_model("B4", _cfg(), vocab_size=32)
    b5 = build_model("B5", _cfg(), vocab_size=32)
    assert isinstance(b4, SGMSLM) and isinstance(b5, SGMSLM)
...
def _train_config(kind="sgms", steps=8, seed=0):
    return {
        "experiment": "test",
        "model": {
            "kind": kind,
...
def test_evaluate_accuracy_range():
    model = build_model("sgms", _cfg(), vocab_size=64)
    task = _task_cfg()
