"""float64 gradcheck over the full SGMS path (spec §9, R4): router + both
...
from sgms.losses import sgms_auxiliary_loss
...
def _block(cfg):
    torch.manual_seed(0)
    return SGMSBlock(cfg, layer_idx=0).to(torch.float64).eval()
...
def test_gradcheck_router_objectives():
    ...
    def fn(weight):
        ...
        total, _ = sgms_auxiliary_loss([routing], cfg.lambda_bal, cfg.lambda_z)
        return total
...
def test_gradcheck_full_block_router_weight_k2():
    ...
    def fn(weight):
        ...
        aux, _ = sgms_auxiliary_loss([routing], 1e-2, 1e-3)
